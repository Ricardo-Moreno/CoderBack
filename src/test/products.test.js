...
describe('Test api/products endpoints', () => {
    ...
    describe('Get All Products', () => {
        ...
        it('GET Should get all products', async () => {
            const { statusCode, ok, body } = await requester.get('/api/products').set('Cookie', [`jwt=${session}`]);
        ...
        it('GET Should get all products with limits', async () => {
            const { statusCode, ok, body } = await requester.get('/api/products?limit=1').set('Cookie', [`jwt=${session}`]);
            expect(ok).to.equal(true);
            expect(statusCode).to.equal(200);
            expect(body.status).to.equal(1);
            expect(body.products).to.be.a('array');
        ...
        it('GET Should get all products with category filter', async () => {
            const { statusCode, ok, body } = await requester.get(`/api/products?category=${testProduct.category}`).set('Cookie', [`jwt=${session}`]);
            expect(ok).to.equal(true);
            expect(statusCode).to.equal(200);
            expect(body.status).to.equal(1);
            expect(body.products).to.be.a('array');
        ...
        it('GET Should not get all products with invalid sort value', async () => {
            const { statusCode, ok, body } = await requester.get('/api/products?sort=invalid').set('Cookie', [`jwt=${session}`]);
    ...
    describe('Get Product by ID', () => {
        ...
        it('GET Should get product by id', async () => {
            const { statusCode, ok, body } = await requester.get(`/api/products/${testProductId}`).set('Cookie', [`jwt=${session}`]);
            expect(ok).to.equal(true);
            expect(statusCode).to.equal(200);
            expect(body.status).to.equal(1);
        ...
        it('GET Should not get product by not existant id', async () => {
            const { statusCode, ok, body } = await requester.get('/api/products/653af750f0cf96e6671536d9').set('Cookie', [`jwt=${session}`]);
            expect(ok).to.equal(false);
        ...
        it('GET Should not get product by invalid id', async () => {
            const { statusCode, ok, body } = await requester.get('/api/products/invalid').set('Cookie', [`jwt=${session}`]);
            expect(ok).to.equal(false);
    ...
    describe('Create Product', () => {
        ...
        it('POST Should create product', async () => {
            const { statusCode, ok, body } = await requester.post('/api/products').set('Cookie', [`jwt=${sessionAdmin}`])
    ...
    describe('Update Product', () => {
        ...
        it('PUT Should update product', async () => {
            const { statusCode, ok, body } = await requester.put(`/api/products/${testProductId}`).set('Cookie', [`jwt=${sessionAdmin}`])
                .send({
                    price: 20,
                });
            expect(ok).to.equal(true);
        ...
        it('PUT Should not update product with invalid id', async () => {
            const { statusCode, ok, body } = await requester.put('/api/products/invalid').set('Cookie', [`jwt=${sessionAdmin}`])
                .send({
                    price: 20,
                });
            expect(ok).to.equal(false);
        ...
        it('PUT Should not update product with not existant id', async () => {
            const { statusCode, ok, body } = await requester.put('/api/products/653af750f0cf96e6671536d9').set('Cookie', [`jwt=${sessionAdmin}`])
                .send({
                    price: 20,
                });
            expect(ok).to.equal(false);
    ...
    describe('Delete Product', () => {
        ...
        it('DELETE Should delete product', async () => {
            const { statusCode, ok, body } = await requester.delete(`/api/products/${testProductId}`).set('Cookie', [`jwt=${sessionAdmin}`]);
            expect(ok).to.equal(true);
            expect(statusCode).to.equal(200);
            expect(body.status).to.equal(1);
        });
        ...
        it('DELETE Should not delete product with invalid id', async () => {
            const { statusCode, ok, body } = await requester.delete('/api/products/invalid').set('Cookie', [`jwt=${sessionAdmin}`]);
            expect(ok).to.equal(false);
            expect(statusCode).to.equal(500);
            expect(body.status).to.equal(0);
            expect(body.errorObject.message).to.include('Cast to ObjectId failed for value');
        ...
        it('DELETE Should not delete product with not existant id', async () => {
            const { statusCode, ok, body } = await requester.delete('/api/products/653af750f0cf96e6671536d9').set('Cookie', [`jwt=${sessionAdmin}`]);
            expect(ok).to.equal(false);
            expect(statusCode).to.equal(404);
            expect(body.status).to.equal(0);
            expect(body.errorObject.message).to.equal('Product not found');
        ...
        it('DELETE Should not delete product with not admin user', async () => {
            const { statusCode, ok, body } = await requester.delete(`/api/products/${newProductId}`).set('Cookie', [`jwt=${session}`]);
            expect(ok).to.equal(false);

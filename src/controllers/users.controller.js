...
const togglePremiumFeature = async (req, res, next) => {
    try {
        ...
        res.send({ status: 1, msg: `Premium ${user.email} successfully toggled. New role is ${user.role}` });
    } catch (error) {
        next(error);
    }
};
...
const updateDocuments = async (req, res, next) => {
    try {
        ...
        res.send({ status: 1, msg: `Documents ${user.email} successfully updated.`, userDocuments: user.documents });
    } catch (error) {
        next(error);
    }
}

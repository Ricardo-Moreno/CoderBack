...
const resetpasswordrequest = async (req, res, next) => {
    try {
        ...
        res.send({ status: 1, msg: `An e-mail to reset the password was successfullysent ${user.email}.` });
    } catch (error) {
        next(error);
    }
};
...
const logout = (req, res) => {
    ...
    if (!jwtCookie) {
        return res.status(400).send({ status: 0, msg: 'Is not logged in.' });
    }
...
const togglePremiumFeature = async (req, res, next) => {
    try {
        ...
        res.send({ status: 1, msg: `Premium feature ${user.email} successfully toggled. New role is ${user.role}` });
    } catch (error) {
        next(error);
    }
};
